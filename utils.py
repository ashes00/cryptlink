...
# Check for cryptography dependency early, as it's needed for fingerprinting
if importlib.util.find_spec("cryptography") is None:
    # Cannot use Tkinter messagebox here easily if main app hasn't started
    print("ERROR: The 'cryptography' library is required but not installed.", file=sys.stderr)
    print(f"Please install it using: '{sys.executable} -m pip install cryptography'", file=sys.stderr)
    sys.exit(1)
